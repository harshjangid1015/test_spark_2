...
object PopularMoviesDataSets {
  ...
  def loadMovieNames() : Map[Int, String] = {
    ...
    var movieNames:Map[Int, String] = Map()
     ...
     val lines = Source.fromFile("../test_spark_2/ml-100k/u.item").getLines()
     for (line <- lines) {
  ...
  def main(args: Array[String]) {
    ...
    val lines = spark.sparkContext.textFile("../test_spark_2/ml-100k/u.data").map(x => Movie(x.split("\t")(1).toInt))
